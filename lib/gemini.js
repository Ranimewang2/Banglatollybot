  ...
  for (const model of models) {
    try {
      ...
      if (text && text.length > 50) {
        logs.push(`[GROQ] ✅ Success with model: ${model} (~${text.split(" ").length} words)`);
        return text;
      } else {
        logs.push(`[GROQ] ⚠️ Empty/short response from ${model}, trying next...`);
        logs.push(`[GROQ] Raw response: ${JSON.stringify(res.data).slice(0, 200)}`);
      }
    } catch (err) {
      const errMsg = err.response?.data?.error?.message || err.message;
      logs.push(`[GROQ] ❌ ${model} failed: ${errMsg}`);
    }
  }
