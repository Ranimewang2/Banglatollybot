...
async function runBot() {
  ...
  const stats = { found: 0, skipped: 0, published: 0, failed: 0 };
  ...
  try {
    ...
    if (todayArticles.length === 0) {
      ...
      logs.push(`[BOT] New to process: ${newArticles.length}`);
      ...
      for (const article of newArticles) {
        ...
        try {
          ...
          if (!scraped.title) {
            logs.push(`[BOT] ❌ No title found anywhere, skipping`);
            stats.failed++;
            continue;
          }
          ...
          if (!articleHtml) { stats.failed++; continue; }

          // Insert video embed
          const finalContent = scraped.videoEmbed
            ? articleHtml.replace("[VIDEOPLACEHOLDER]", scraped.videoEmbed)
            : articleHtml.replace("[VIDEOPLACEHOLDER]", "");
      ...
      // Step 5: Save processed URLs
      if (newlyProcessed.length > 0) {
        await saveProcessedUrls([...processedUrls, ...newlyProcessed]);
        logs.push(`[DB] ✅ Saved ${newlyProcessed.length} new URLs`);
      }
    }
  ...
  logs.push(`[BOT] Found:${stats.found} | Skipped:${stats.skipped} | Published:${stats.published} | Failed:${stats.failed}`);

  // Save run to logs bin
  const runData = {
